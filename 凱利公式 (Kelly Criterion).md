...
為了找出使 $G$ 最大的 $f$，我們對 $f$ 微分並令其為 0
$$\frac{dG}{df} = \frac{p \gamma}{1 + \gamma f} - \frac{q}{1 - f} = 0$$
通分整理：
...
1. 問題設定：
	* 贏的賠率 ($\gamma$)：賺的錢是普通虧損的幾倍？
$$\gamma = \frac{889.12}{1366.32} \approx 0.651$$
意義：每承擔 1 單位的普通風險，獲利時可以賺 0.651 單位
	* 毀滅性虧損倍數 ($\mu$)：
大賠的錢是普通虧損的幾倍？
$$\mu = \frac{2637.00}{1366.32} \approx 1.930$$
...
計算單位期望值 ：
$$E_{gain} = (\text{贏的獲利} \times p_1) - (\text{普通虧損} \times p_2) - (\text{大賠虧損} \times p_3)$$  
$$E_{gain} = (0.651 \times 0.653) - (1 \times 0.32) - (1.930 \times 0.026)$$
(註：普通虧損倍數為 1)
